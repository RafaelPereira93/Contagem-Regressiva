...
const handleSubmit = (event) => {
  ...
  const { days } = countdown(inputValue);

  if (verificarSeADataEValida(days)) {
    resetarCampoDeInput();
    return;
  }
  ...
  const temporizadorData = setInterval(() => {
    const { days, hours, minutes, seconds } = countdown(inputValue);
    inserirDataNoDom(days, hours, minutes, seconds);
  }, 1000);
